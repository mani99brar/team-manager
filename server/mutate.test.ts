...
test('create-file creates an empty or filled Markdown file in either source, nested parents and uppercase .MD included', async () => {
  await withTempRoot(async root => {
    ...
    try {
      const empty = await mutate(app, { op: 'create-file', source: 'Pi', path: 'new.md' })
      assert.equal(empty.statusCode, 201, empty.body)
      assert.deepEqual(empty.json(), { op: 'create-file', source: 'Pi', path: 'new.md' })
      assert.equal(await readFile(join(root, 'pi', 'new.md'), 'utf8'), '')

      const filled = await mutate(app, { op: 'create-file', source: 'Claude', path: 'nested/deeper/NOTE.MD', content: '# Hello\r\n' })
      assert.equal(filled.statusCode, 201, filled.body)
      assert.deepEqual(filled.json(), { op: 'create-file', source: 'Claude', path: 'nested/deeper/NOTE.MD' })
      ...
      const duplicate = await mutate(app, { op: 'create-file', source: 'Claude', path: 'new.md', content: 'claude' })
      ...
      const special = await mutate(app, { op: 'create-file', source: 'Pi', path: 'a b#c?d%2e.md' })
      ...
      const read = await app.inject('/api/file?source=Claude&path=nested%2Fdeeper%2FNOTE.MD')
...
test('create-folder creates one empty directory and never creates parents implicitly', async () => {
  await withTempRoot(async root => {
    const app = createApp(root)
    try {
      const created = await mutate(app, { op: 'create-folder', source: 'Claude', path: 'topics' })
      assert.equal(created.statusCode, 201, created.body)
      assert.deepEqual(created.json(), { op: 'create-folder', source: 'Claude', path: 'topics' })
      assert.ok((await stat(join(root, 'claude', 'topics'))).isDirectory())
      const nested = await mutate(app, { op: 'create-folder', source: 'Claude', path: 'topics/inner' })
      assert.equal(nested.statusCode, 201)

      const missingParent = await mutate(app, { op: 'create-folder', source: 'Pi', path: 'missing/child' })
      assert.equal(missingParent.statusCode, 404)
      assert.equal(missingParent.json().code, 'NOT_FOUND')
      const missingFileParent = await mutate(app, { op: 'create-file', source: 'Pi', path: 'missing/child.md' })
...
test('rename changes the basename within the same parent for files and empty folders', async () => {
  await withTempRoot(async root => {
    ...
    const app = createApp(root)
    try {
      const file = await mutate(app, { op: 'rename', source: 'Pi', path: 'skills/review.md', destinationPath: 'skills/Reviewed.MD' })
      assert.equal(file.statusCode, 200, file.body)
      assert.deepEqual(file.json(), { op: 'rename', source: 'Pi', path: 'skills/review.md', destinationSource: 'Pi', destinationPath: 'skills/Reviewed.MD' })
      ...
      const folder = await mutate(app, { op: 'rename', source: 'Pi', path: 'empty', destinationPath: 'renamed' })
      ...
      const elsewhere = await mutate(app, { op: 'rename', source: 'Pi', path: 'skills/Reviewed.MD', destinationPath: 'Reviewed.MD' })
      ...
      const extension = await mutate(app, { op: 'rename', source: 'Pi', path: 'skills/Reviewed.MD', destinationPath: 'skills/Reviewed.txt' })
      assert.equal(extension.statusCode, 400)
      const folderExtension = await mutate(app, { op: 'rename', source: 'Pi', path: 'renamed', destinationPath: 'renamed.md' })
      assert.equal(folderExtension.statusCode, 400)
      // Same path is a no-op conflict.
      const same = await mutate(app, { op: 'rename', source: 'Pi', path: 'skills/Reviewed.MD', destinationPath: 'skills/Reviewed.MD' })
...
test('move relocates files and empty folders within the same source; the destination parent must exist', async () => {
  await withTempRoot(async root => {
    ...
    const app = createApp(root)
    try {
      const file = await mutate(app, { op: 'move', source: 'Claude', path: 'a/doc.md', destinationPath: 'b/moved.md' })
      assert.equal(file.statusCode, 200, file.body)
      assert.deepEqual(file.json(), { op: 'move', source: 'Claude', path: 'a/doc.md', destinationSource: 'Claude', destinationPath: 'b/moved.md' })
      const folder = await mutate(app, { op: 'move', source: 'Claude', path: 'a/empty', destinationPath: 'empty' })
      assert.equal(folder.statusCode, 200, folder.body)
      assert.deepEqual(await tree(join(root, 'claude')), ['a/', 'b/', 'b/moved.md', 'empty/'])

      const missingParent = await mutate(app, { op: 'move', source: 'Claude', path: 'b/moved.md', destinationPath: 'nope/moved.md' })
      assert.equal(missingParent.statusCode, 404)
      assert.equal(missingParent.json().code, 'NOT_FOUND')
      const intoItself = await mutate(app, { op: 'move', source: 'Claude', path: 'b', destinationPath: 'b/inner' })
      assert.equal(intoItself.statusCode, 400)
      const crossSource = await mutate(app, { op: 'move', source: 'Claude', path: 'b/moved.md', destinationSource: 'Pi', destinationPath: 'moved.md' })
...
test('delete removes a Markdown file or an empty folder and never recurses', async () => {
  await withTempRoot(async root => {
    ...
    const app = createApp(root)
    try {
      const file = await mutate(app, { op: 'delete', source: 'Pi', path: 'gone.md' })
      assert.equal(file.statusCode, 200, file.body)
      assert.deepEqual(file.json(), { op: 'delete', source: 'Pi', path: 'gone.md' })
      const folder = await mutate(app, { op: 'delete', source: 'Pi', path: 'empty' })
      assert.equal(folder.statusCode, 200)
      for (const path of ['hidden-only', 'other-only', 'nested']) {
        const response = await mutate(app, { op: 'delete', source: 'Pi', path })
      ...
      const again = await mutate(app, { op: 'delete', source: 'Pi', path: 'gone.md' })
...
test('nonempty folders cannot be renamed or moved either', async () => {
  await withTempRoot(async root => {
    ...
    const app = createApp(root)
    try {
      const rename = await mutate(app, { op: 'rename', source: 'Pi', path: 'full', destinationPath: 'renamed' })
      assert.equal(rename.statusCode, 409)
      assert.equal(rename.json().code, 'FOLDER_NOT_EMPTY')
      await mkdir(join(root, 'pi', 'target'))
      const move = await mutate(app, { op: 'move', source: 'Pi', path: 'full', destinationPath: 'target/full' })
...
test('copy duplicates the exact bytes of a Markdown file into the other source only', async () => {
  await withTempRoot(async root => {
    ...
    const app = createApp(root)
    try {
      const copied = await mutate(app, { op: 'copy', source: 'Pi', path: 'skills/review.md', destinationSource: 'Claude', destinationPath: 'imported/review.md' })
      assert.equal(copied.statusCode, 201, copied.body)
      assert.deepEqual(copied.json(), { op: 'copy', source: 'Pi', path: 'skills/review.md', destinationSource: 'Claude', destinationPath: 'imported/review.md' })
      ...
      const sameSource = await mutate(app, { op: 'copy', source: 'Pi', path: 'skills/review.md', destinationSource: 'Pi', destinationPath: 'copy.md' })
      assert.equal(sameSource.statusCode, 400)
      const noDestination = await mutate(app, { op: 'copy', source: 'Pi', path: 'skills/review.md', destinationPath: 'copy.md' })
      assert.equal(noDestination.statusCode, 400)
      const folder = await mutate(app, { op: 'copy', source: 'Pi', path: 'skills', destinationSource: 'Claude', destinationPath: 'skills' })
      assert.equal(folder.statusCode, 404)
      const missingParent = await mutate(app, { op: 'copy', source: 'Pi', path: 'skills/review.md', destinationSource: 'Claude', destinationPath: 'nope/review.md' })
      assert.equal(missingParent.statusCode, 404)
      const extension = await mutate(app, { op: 'copy', source: 'Pi', path: 'skills/review.md', destinationSource: 'Claude', destinationPath: 'imported/review.txt' })
      assert.equal(extension.statusCode, 400)
  ...
  await withTempRoot(async root => {
    ...
    try {
      const cases: Array<[string, unknown]> = [
        ['create-file over file', { op: 'create-file', source: 'Pi', path: 'taken.md' }],
        ['create-file over dangling symlink', { op: 'create-file', source: 'Pi', path: 'dangling.md' }],
        ['create-folder over folder', { op: 'create-folder', source: 'Pi', path: 'taken-folder' }],
        ['create-folder over dangling symlink', { op: 'create-folder', source: 'Pi', path: 'dangling-folder' }],
        ['create-folder over file', { op: 'create-folder', source: 'Pi', path: 'plain' }],
        ['create-file over folder', { op: 'create-file', source: 'Pi', path: 'folder-named.md' }],
        ['rename onto file', { op: 'rename', source: 'Pi', path: 'source.md', destinationPath: 'taken.md' }],
        ['rename onto dangling symlink', { op: 'rename', source: 'Pi', path: 'source.md', destinationPath: 'dangling.md' }],
        ['move onto file', { op: 'move', source: 'Pi', path: 'source.md', destinationPath: 'taken.md' }],
        ['folder rename onto folder', { op: 'rename', source: 'Pi', path: 'empty', destinationPath: 'taken-folder' }],
        ['folder rename onto dangling symlink', { op: 'rename', source: 'Pi', path: 'empty', destinationPath: 'dangling-folder' }],
        ['copy onto file', { op: 'copy', source: 'Pi', path: 'source.md', destinationSource: 'Claude', destinationPath: 'taken.md' }],
      ]
...
test('invalid or missing fields, unknown ops, bad sources and malformed paths are rejected with 400 and no disk change', async () => {
  await withTempRoot(async root => {
    ...
    try {
      const cases: Array<[string, unknown]> = [
        ['array', [{ op: 'create-file', source: 'Pi', path: 'x.md' }]],
        ['null', 'null'],
        ['no op', { source: 'Pi', path: 'x.md' }],
        ['unknown op', { op: 'truncate', source: 'Pi', path: 'real.md' }],
        ['op case', { op: 'Create-File', source: 'Pi', path: 'x.md' }],
        ['missing source', { op: 'create-file', path: 'x.md' }],
        ['bad source', { op: 'create-file', source: 'pi', path: 'x.md' }],
        ['missing path', { op: 'create-file', source: 'Pi' }],
        ['empty path', { op: 'create-file', source: 'Pi', path: '' }],
        ['numeric path', { op: 'create-file', source: 'Pi', path: 3 }],
        ['non-string content', { op: 'create-file', source: 'Pi', path: 'x.md', content: 1 }],
        ['create non-md', { op: 'create-file', source: 'Pi', path: 'x.txt' }],
        ['create md folder', { op: 'create-folder', source: 'Pi', path: 'x.md' }],
        ['traversal', { op: 'create-file', source: 'Pi', path: '../x.md' }],
        ['absolute', { op: 'create-file', source: 'Pi', path: '/x.md' }],
        ['backslash', { op: 'create-file', source: 'Pi', path: 'a\\x.md' }],
        ['NUL', { op: 'create-file', source: 'Pi', path: 'x.md\0' }],
        ['dot', { op: 'create-folder', source: 'Pi', path: '.' }],
        ['drive', { op: 'create-folder', source: 'Pi', path: 'C:/x' }],
        ['UNC', { op: 'create-folder', source: 'Pi', path: '\\\\server\\share' }],
        ['trailing slash', { op: 'create-folder', source: 'Pi', path: 'x/' }],
        ['rename missing destination', { op: 'rename', source: 'Pi', path: 'real.md' }],
        ['rename numeric destination', { op: 'rename', source: 'Pi', path: 'real.md', destinationPath: 5 }],
        ['rename traversal destination', { op: 'rename', source: 'Pi', path: 'real.md', destinationPath: '../real.md' }],
        ['rename absolute destination', { op: 'rename', source: 'Pi', path: 'real.md', destinationPath: '/tmp/real.md' }],
        ['rename to empty', { op: 'rename', source: 'Pi', path: 'real.md', destinationPath: '' }],
        ['move traversal destination', { op: 'move', source: 'Pi', path: 'real.md', destinationPath: 'a/../../real.md' }],
        ['move destination source mismatch', { op: 'move', source: 'Pi', path: 'real.md', destinationSource: 'Claude', destinationPath: 'real.md' }],
        ['copy bad destination source', { op: 'copy', source: 'Pi', path: 'real.md', destinationSource: 'Nope', destinationPath: 'real.md' }],
        ['copy traversal destination', { op: 'copy', source: 'Pi', path: 'real.md', destinationSource: 'Claude', destinationPath: '../real.md' }],
        ['copy NUL destination', { op: 'copy', source: 'Pi', path: 'real.md', destinationSource: 'Claude', destinationPath: 'x\0.md' }],
        ['delete non-md file', { op: 'delete', source: 'Pi', path: 'notes.txt' }],
      ]
      for (const [label, payload] of cases) {
...
test('source roots cannot be renamed, moved, deleted or copied, and an empty path is never a target', async () => {
  await withTempRoot(async root => {
    ...
    try {
      const cases: unknown[] = [
        { op: 'rename', source: 'Pi', path: '', destinationPath: 'renamed' },
        { op: 'move', source: 'Pi', path: '', destinationPath: 'moved' },
        { op: 'delete', source: 'Pi', path: '' },
        { op: 'copy', source: 'Pi', path: '', destinationSource: 'Claude', destinationPath: 'x.md' },
        { op: 'move', source: 'Pi', path: 'real.md', destinationPath: '' },
        { op: 'create-folder', source: 'Pi', path: '' },
      ]
...
test('missing targets and symlinks anywhere in source or destination paths are rejected with 404 and nothing changes', async () => {
  await withTempRoot(async root => {
    ...
    try {
      const cases: Array<[string, unknown]> = [
        ['delete missing', { op: 'delete', source: 'Pi', path: 'missing.md' }],
        ['rename missing', { op: 'rename', source: 'Pi', path: 'missing.md', destinationPath: 'x.md' }],
        ['move missing folder', { op: 'move', source: 'Pi', path: 'missing', destinationPath: 'x' }],
        ['copy missing', { op: 'copy', source: 'Pi', path: 'missing.md', destinationSource: 'Claude', destinationPath: 'x.md' }],
        ['delete symlinked file', { op: 'delete', source: 'Pi', path: 'linked.md' }],
        ['rename symlinked file', { op: 'rename', source: 'Pi', path: 'linked.md', destinationPath: 'renamed.md' }],
        ['delete symlinked dir', { op: 'delete', source: 'Pi', path: 'linked-dir' }],
        ['copy symlinked file', { op: 'copy', source: 'Pi', path: 'linked.md', destinationSource: 'Claude', destinationPath: 'x.md' }],
        ['create through symlinked dir', { op: 'create-file', source: 'Pi', path: 'linked-dir/new.md' }],
        ['create through self link', { op: 'create-folder', source: 'Pi', path: 'self/new' }],
        ['delete through symlinked dir', { op: 'delete', source: 'Pi', path: 'linked-dir/secret.md' }],
        ['move into symlinked dir', { op: 'move', source: 'Pi', path: 'real.md', destinationPath: 'linked-dir/real.md' }],
        ['move into self link', { op: 'move', source: 'Pi', path: 'real.md', destinationPath: 'self/real.md' }],
        ['copy into symlinked source root', { op: 'copy', source: 'Pi', path: 'real.md', destinationSource: 'Claude', destinationPath: 'real.md' }],
        ['create in symlinked source root', { op: 'create-file', source: 'Claude', path: 'new.md' }],
        ['delete in symlinked source root', { op: 'delete', source: 'Claude', path: 'secret.md' }],
        ['delete file as folder parent', { op: 'delete', source: 'Pi', path: 'real.md/child.md' }],
      ]
      for (const [label, payload] of cases) {
        const response = await mutate(app, payload)
...
test('a case-only rename succeeds when the filesystem reports the destination absent and is refused when it reports an entry', async () => {
  await withTempRoot(async root => {
    ...
    try {
      ...
      const renamed = await mutate(app, { op: 'rename', source: 'Pi', path: 'Notes.md', destinationPath: 'notes.md' })
      ...
      const refused = await mutate(app, { op: 'rename', source: 'Pi', path: 'notes.md', destinationPath: 'NOTES.md' })
  ...
  await withTempRoot(async root => {
    ...
    try {
      const creates = await Promise.all([
        mutate(app, { op: 'create-file', source: 'Pi', path: 'same.md', content: 'one' }),
        mutate(app, { op: 'create-file', source: 'Pi', path: 'same.md', content: 'two' }),
      ])
      ...
      const renames = await Promise.all([
        mutate(app, { op: 'rename', source: 'Pi', path: 'a.md', destinationPath: 'target.md' }),
        mutate(app, { op: 'rename', source: 'Pi', path: 'b.md', destinationPath: 'target.md' }),
      ])
      ...
      const [save, remove] = await Promise.all([
        app.inject({ method: 'PUT', url: '/api/file', payload: { source: 'Pi', path: 'saved.md', content: 'updated', expectedHash: sha256('saved') } }),
        mutate(app, { op: 'delete', source: 'Pi', path: 'saved.md' }),
      ])
...
test('unexpected filesystem failures are safe 500 errors without paths', async () => {
  await withTempRoot(async root => {
    ...
    try {
      ...
      const response = await mutate(app, { op: 'create-folder', source: 'Pi', path: 'new' })
...
test('the committed fixtures are never changed by the mutation test suite', async () => {
  ...
  try {
    ...
    const response = await mutate(app, { op: 'delete', source: 'Pi', path: 'skills' })
