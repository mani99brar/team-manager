...
The scaffold is a 2.1.0 `feature.json` with one lane and the bundled reviewers, a 1.2.0 `policy.json`
with a placeholder check, the lane's task in outcome-brief form and a `README.md`, plus a starter
`CLAUDE.md` in the target root when it has none. Every value to decide is a `TODO:` placeholder, and
`launch` refuses the feature, naming each one, until none is left.
"""
...
def feature_files(feature: str) -> dict[str, str]:
    ...
    manifest = {"version": "2.1.0", "name": f"TODO: one line saying what {feature} delivers", "branch_prefix": f"feature/{feature}",
    ...
    readme = (f"# {feature}\n\n"
              ...
              "- `feature.json`: the lanes, their task files and the reviewers (`builtin:<id>` names a bundled brief).\n"
              "- `policy.json`: each lane's owned paths and the checks the controller runs independently.\n"
              f"- `{LANE}-task.md`: the lane's task as an outcome brief.\n\n"
    ...
    return {"feature.json": json.dumps(manifest, indent=2) + "\n", "policy.json": json.dumps(policy, indent=2) + "\n",
            f"{LANE}-task.md": task, "README.md": readme}
