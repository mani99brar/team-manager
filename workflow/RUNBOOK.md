...
- **Failed verification:** inspect its packet/log. Attempts default to a hard limit of three per lane, phase and revision (a lane repair starts a new revision, see below); policy v1.1.0 and later can explicitly set `max_verification_attempts`. For a transient check/environment failure at the same immutable revision, explicitly retry only that lane (`--node` accepts any lane of the run and nothing else):
  ...
  On an automatic run the supervisor retries a failed check by itself within the limit; `retry --phase/--node` is for a check it stopped at, such as an interrupted one. There it only raises the attempt and prints `automatic "$RUN" --live`, whose controller runs the check at that attempt: invoked by `retry`, the graph would run the review node, reviewer launches included, in the CLI process.
...
- **Worktree creation failed:** the error ends with Git's own message. `cannot lock` or `.lock': File exists` after the retries means another Git process holds that lock file, or a crashed one left it behind: make sure none is running, delete the named `.lock` file, then rerun what failed: a check as above (its attempt directory has no packet, which `automatic --live` never retries by itself), the design challenge's checkout with `resume` (Guardrails).
- **Failure after stopping/snapshotting or integrating:** `retry "$RUN"` resumes only failed graph steps; it never launches a new Claude worker. On an automatic run it carries the rest of the run in that process, the reviewers included when review is still ahead: run it in a persistent pane. Stop-intent recovery checks whether the prior stop already completed before issuing another native stop.
...
| `retry` | | `--phase worker\|candidate`, `--node <lane>` | reruns one failed check at the same revision (on an automatic run `--node` only raises its attempt, and `automatic --live` reruns it), or resumes failed post-freeze steps; never relaunches |
