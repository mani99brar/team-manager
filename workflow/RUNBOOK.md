...
A policy may declare a `failure_drill`: a deliberate first verification failure of one lane, retried explicitly ("Status, failures and recovery"). Every lane/phase has a hard limit of three verification attempts per revision (a lane repair starts a new one). Automatic mode adds persisted per-worker deadlines (default 4 hours, `--worker-timeout-seconds`) enforced by the running supervisor and a reviewer deadline (default 30 minutes, `--review-timeout-seconds`, counted from the reviewer's launch). Interrupting the supervisor leaves the workers running and the run resumable, and so does Claude Code itself being unavailable, an update respawning a session under a new PID included (exit 75, see "Status, failures and recovery"); only deadline expiry, a worker that writes `status: blocked`, or a native session that is gone (`stopped` or `failed`, or listed under another identity) stops them. A worker whose native state is `blocked` (its turn ended on a question, a permission prompt or a refusal the harness could not continue past) is not a failure: the controller records one `interactive` event naming the lane, keeps the other lanes running and waits until that worker's deadline (for a lane whose completion was already accepted, the bound in "Worker questions"), so the operator can answer in its pane. It does not impose token caps or automatically repair code. Manual mode retains operator-controlled worker lifetimes. The independent review runs as a third native session by default; `--reviewer-transport print` keeps the headless `claude --print` reviewer for environments without Herdr (see "Automatic mode: the review step").
...
- **Override:** `"$PY" -m workflow resume "$RUN" --accept-challenge "<reason>" [--herdr]` rewrites the paused attempt as `accepted` with the reason (the paused record stays as `challenge-<n>.json`), reruns nothing and launches the workers. The reason is shown in the viewer. It refuses while a pinned feature file no longer holds the plan's copy, edited or committed by hand (the workers would run without the edit: rerun without `--accept-challenge` to use it, or revert it; a task file is compared as written, so an override from another interpreter or checkout of the tool than prepare's is no edit), while an interrupted `resume` has not finished (`challenge-revision.json`, or a revision commit the run does not use yet), and when the paused attempt read other files than the plan now pins (a rerun failed after its re-pin; the digests in `challenge.json` `pinned` differ): rerun without `--accept-challenge` so a challenge reads them.
...
**Worker questions.** A worker that cannot decide alone writes its completion file with `status: question`; once its turn is over (the session idle, done or `blocked`: a turn that ends waiting on the operator usually reports `blocked`, and needs no other attention event) the controller keeps it as `<lane>.question-<n>.json`, appends `{n, question, asked_at, answer: null, answered_at: null}` to `<lane>.questions.json`, records an `interactive` event with the question and the `answer` command, and pauses that lane's deadline (`<lane>.deadline.json`: `paused_at` while waiting, `paused_seconds` accumulated). The pause starts when the worker wrote the file (its modification time, never before the lane's deadline last ran again), so a question the controller first reads after the deadline, because it was away (exit 75, Ctrl-C), still pauses it; one written after the deadline is refused as before. The other lanes and their deadlines keep running; a controller restart reads the same pause. A deadline runs from launch to the completion signal: a lane whose completion file was accepted (its session idle or done) has none while another lane works or waits on a question, so a lane that finished early never ends the run while another lane waits on its answer. Once every lane's completion was accepted, a lane whose session works or blocks again (an operator prompt in its pane, a command of its own) is held to the latest lane deadline (launch plus `worker_timeout_seconds` plus the lane's paused time, the latest over the run's lanes): an `interactive` event names that time, a turn that ends before it is handed off as usual, and past it the run stops like any deadline. Answer with:

```bash
"$PY" -m workflow answer "$RUN" <lane> "Use option B; record the trade-off as an open assumption."
```

It refuses when the lane's latest question is answered or the worker went on from it (a completion file written since, its saved handoff, its stop: the pane of a stopped worker is a shell, which would run the text), records the answer and its time (with `delivered: false`), restarts the deadline (the paused time is added to it) and types the text into the lane's pane from `terminals.json` (`herdr pane send-text <pane> <text>`, recorded as `typed: true`, then `herdr pane send-keys <pane> Enter`; needs `HERDR_ENV=1`), then sets `delivered: true`. It types only while `herdr pane process-info` shows `claude attach <id>` of the lane's session in that pane's foreground: once attach-one ended (Ctrl+Z detaches to the pane's shell, which would run the text; an interrupt; a give-up), or while it waits between two attaches, nothing is typed and the delivery fails; run attach-one in the pane again, then rerun `answer`. With `--no-herdr` it prints `claude attach <id>` so you can type the answer yourself; that counts as the delivery. If the delivery fails (outside a Herdr pane, a run started without `--herdr` so no pane is recorded, a closed pane, a pane that does not show the session), it exits 1 with the reason: the answer stays recorded and the deadline keeps running (a deadline paused until a delivery would never run again if you then typed the answer in the pane), but the worker has not received it. Rerun one of the two commands it prints (the same command from a Herdr pane, or with `--no-herdr`): it delivers the recorded answer, typing the text at most once, and records nothing again (refused, and nothing typed, once the worker went on). After a failed Enter the text was typed into the session's input: the rerun presses Enter only, and only while `herdr pane read` shows it in the pane's input line (`❯` and the text between two rules). A session an update respawned (a new PID) starts with an empty input: a pane showing the input empty, other text or no input line (a dialog) gets no Enter, and the answer stays undelivered. Look at the pane, then rerun with `--no-herdr` (it prints what to do and records the delivery) and press Enter there, typing the answer first if the input does not hold it. When Herdr does not confirm the text in time it may be in the input anyway: look at the pane before rerunning, and if the input holds it, press Enter there instead. A different text for that question is refused, and so is any rerun once the answer was delivered; if you type the answer in the pane after a failed delivery, do not rerun `answer` too. If you type into the pane directly instead, the controller records `(no answer recorded: the worker's session worked again in its pane)` as soon as it sees the session work again, restarts the deadline and says so in an `interactive` event. It sees that when a poll finds the session `working`, or else when the worker writes its next completion signal: the registry may report a whole reply turn as `blocked`, or keep `done`, and the deadline stays paused until then. A session can also work again with nobody answering (a background command it started ended): until the worker writes its next completion signal, `answer` still records your text over that placeholder and types it, and the deadline keeps running. Once you typed the answer in the pane yourself, do not run `answer` too: it would type it again. A worker gets at most three answered questions; a fourth `question` blocks the run like `status: blocked` (the file stays as evidence): the `controller` event quotes the question, and the viewer shows that lane as blocked with its text.
...
- **Deadline.** `review_timeout_seconds` counts from each reviewer's own launch (`launch_requested_at`) and survives controller restarts. A reviewer idle or done with a valid file is accepted before its deadline is checked, so a verdict the controller reads again after the deadline (resumed after exit 75 or Ctrl-C) still counts. A reviewer whose file was accepted before the controller stopped (`accepted_at` in its status, saved with the `accepted_decision` and the file's `completion_sha256`) keeps that decision whatever its session does now (a follow-up typed in its pane), as a controller that never stopped would not have waited for it or read its file again: a file rewritten, half written or removed since is not read, and a `running` event says it changed after acceptance. A status saved by an older controller, without `accepted_decision`, is read again, and a file that no longer validates blocks as on its first read. The first reviewer to idle without a file until its deadline blocks the run; verdicts already accepted from the others are retained in `review.json`; **no second reviewer is ever launched**.
...
- **Interruption.** Ctrl-C, a closed terminal or a dropped SSH session while the controller waits leaves every reviewer running with `automatic-review.json` at `running`; `python -m workflow automatic "$RUN" --live` resumes waiting for the same sessions' files. The same holds inside a launch window once that reviewer's `claude --bg` was issued (its `.interactive.json` exists): the receipt is `running`, without `session_id` when the interrupt hit the settle poll, and resume rebinds that one session through reconciliation, never a relaunch; Claude Code unavailable during that rebind leaves the review resumable the same way (exit 75). An interrupt before a reviewer's launch was issued leaves the run at `needs_reconciliation`: resume rebinds the reviewers that were launched and launches nothing, so the run cannot continue; stop the running reviewers by their exact ids and start a new run.
...
- **Failed verification:** inspect its packet/log. Attempts default to a hard limit of three per lane, phase and revision (a lane repair starts a new revision, see below); policy v1.1.0 and later can explicitly set `max_verification_attempts`. For a transient check/environment failure at the same immutable revision, explicitly retry only that lane (`--node` accepts any lane of the run and nothing else):
  ...
  On an automatic run the supervisor retries a failed check by itself within the limit; `retry --phase/--node` is for a check step it stopped at without a verdict (an interrupted check, one whose worktree Git could not create, a candidate step that failed before its checks, for which one lane without a candidate packet is enough). There it only raises the attempt and prints `automatic "$RUN" --live`, whose controller runs the step at that attempt: invoked by `retry`, the graph would run the review node, reviewer launches included, in the CLI process.
...
- **Worktree creation failed:** the error ends with Git's own message. `cannot lock` or `.lock': File exists` after the retries means another Git process holds that lock file, or a crashed one left it behind: make sure none is running, delete the named `.lock` file, then rerun what failed: a check or the candidate step with `retry` as above (it left no packet, which `automatic --live` never retries by itself), the design challenge's checkout with `resume` (Guardrails), the review worktree of an automatic run with `automatic "$RUN" --live`: its controller re-enters a review that launched no reviewer once per `automatic --live` (the timeline says `Re-entering the review`; `review-restart.json` keeps it to once across the supervisor's step processes), and stops if the same failure comes back. A `review-worktree/` left behind by a failure after its creation stops it with "Partial review worktree … remove it with git worktree remove --force …": remove it, then rerun `automatic --live`.
- **Failure after stopping/snapshotting or integrating:** `retry "$RUN"` resumes only failed graph steps; it never launches a new Claude worker. On an automatic run it refuses until the review is recorded (`review.json`), since it would run the review, reviewer launches included, in its own process: before that `automatic --live` continues the run; after it (a failed integration) `retry` finishes it and launches nothing. Stop-intent recovery checks whether the prior stop already completed before issuing another native stop.
...
- **Claude Code unavailable (exit 75):** an update replacing the `claude` binary, or its background service restarting, is not a verdict on any session. Every `claude` call the controller makes waits it out for 60 seconds: a launch, a print job or a stop is repeated only when its exec failed (nothing ran, so none ever starts twice), and the `claude agents --json` listing, which only reads, is asked again every 2 seconds while it fails, prints no list or hangs (each 15-second timeout counts against the 60). After that the automatic controller stops nothing: it records an `interrupted` event naming the cause, and `automatic` (also `launch --automatic` and `resume`) exits 75 with the resume command and any stale Claude processes to restart (its `automatic-step` child exits 69, EX_UNAVAILABLE, which the supervisor turns into that 75, whichever step was running; 75 from a step still means a checkpoint persisted). The service also respawns idle sessions (a finished lane, a lane paused on a question, a reviewer that wrote its file) under new PIDs, about 15 seconds after it restarts or at any later time: while `claude agents` lists such a session without a PID, in a state between two processes, with its ended PID or not at all, the worker and reviewer waits, the reviewers' identity check before the verdict and every stop look again every 2 seconds, and a stop follows the new PID (same background id and session UUID); a gap that lasts 30 seconds counts as Claude Code unavailable. Only a stop already issued (`issued` in `<node>.stop.json`, written right before `claude stop` runs, and cleared when it exits non-zero, times out or cannot be executed, as it can while the service restarts; a Ctrl-C while it runs leaves it set) counts a session the listing leaves out as stopped; a failed stop is issued again to the respawned session. Once `claude --version` works, run `"$PY" -m workflow automatic "$RUN" --live`: it waits for the same sessions again (a review interrupted this way re-enters its node, rebinding the running reviewers; a freeze re-enters, marked by `freeze-interrupted.json`, completing the stops it recorded in `<lane>.stop.json`; each marker is removed only once that re-entry ends, so a Ctrl-C or a closed terminal during it leaves the run resumable the same way); nothing is relaunched. A reviewer launch it interrupts leaves the review at `needs_reconciliation` (below), and a print review it interrupts terminates its jobs: no resume continues either, so the controller records a `blocked` event naming the cause and `automatic` exits 1. A deadline, a `blocked` completion, a rejected file, a `stopped` or `failed` session or a changed session identity still stops the sessions: every lane's stop is attempted, and the ones that cannot be confirmed (usually that lane's own) are named together on the timeline. A session still missing after 30 seconds makes every `automatic --live` exit 75 the same way: `claude agents --json --all` and `claude logs <id>` show whether it ended; waking it with `claude attach <id>`, or stopping the run (below), is your decision.
...
- **Refused** (exit 1, `Blocked:`, nothing written): a run not stopped at a check verdict after freeze (`reconcile` a launch; an interrupted check, a check step that failed without a verdict such as an infrastructure failure, or a run stopped between steps: run what the refusal prints for the run's mode, on a manual run `retry` (`--phase <p> --node <lane>` for an interrupted check), on an automatic run `automatic --live`, after `retry --phase <p> --node <lane>` for a check step), a lane blocked before freeze (an automatic run then needs a new run), a started review, a held lock, a moved, dirty or switched source checkout, a fourth repair and a plan without configured lanes. An applied repair is not replaced before the run continues from it: check it with `--dry-run` first. Once the continuation stops at a check verdict again (the repaired lane's own check included), the next repair builds on it.
...
| `answer` | `<lane> "<text>"` | `--no-herdr` | 2.2.0: records the answer to the lane's waiting question, restarts its deadline and types it into its pane (or prints `claude attach <id>`); rerun after a failed delivery, it delivers the recorded answer, typing the text at most once |
...
| `repair` | `<lane>[,<lane>]` and `--commit <sha> --reason "<text>"`, or `--workspace` | `--dry-run` | after freeze, before review: a detached worktree at the fix's base, or the fix commit as new lane snapshots and a checkpoint forked at the freeze boundary; launches nothing, runs no check; continue with `automatic --live` (a manual run: `retry`) |
| `retry` | | `--phase worker\|candidate`, `--node <lane>` | reruns one failed check at the same revision (on an automatic run `--node` only raises its attempt, and `automatic --live` reruns it), or resumes failed post-freeze steps (on an automatic run only once its review is recorded); never relaunches |
...
| `<lane>.question-<n>.json`, `<lane>.questions.json`, `<lane>.deadline.json`, `questions.lock` | controller, `answer` | a worker's question files, their answers and times (and whether `answer` typed and delivered them), the persisted deadline pause and `met_at`: the lane's completion signal was accepted with its turn ended while other lanes still worked, so its deadline stays met if its session works again, until every lane's was accepted (then the latest lane deadline bounds it) |
...
| `<node>.stop.json` | stop | identity-checked stop marker (`issued` once `claude stop` runs, cleared if it fails or times out) |
| `review-restart.json` | controller | the error of a review that failed before any reviewer launch and was re-entered; each `automatic --live` clears it when it starts, so a review is re-entered at most once per supervisor |
| `freeze-interrupted.json` | controller | the cause of a freeze Claude Code's unavailability interrupted; the next `automatic --live` re-enters the freeze and removes it once that re-entry ends |
...
- **Ctrl+Z** detaches to the pane's shell; the background session keeps running, and `answer` types into that pane only once attach-one runs in it again. Closing a pane or losing SSH does not stop the worker. **Ctrl+C inside Claude** interrupts its current turn. Do not type `/exit` unless you mean to end the session.
