...
- `sessions.py`: run preparation (one worktree per selected lane), lane id rules, receipts, locking, and the `claude` process helpers (`run_claude`, `popen_claude`, `exec_claude`: the auto-updater off, an update in progress waited out) with the stale-process warning.
