...
Workers emit an explicit run/node/token-bound completion file. Idle alone is insufficient. The controller freezes completed work, executes the real isolated and combined checks, and launches a fresh native reviewer session from the LangGraph review node: a third pane `Claude: reviewer` in the run's tab, read-only tools, its own `review-worktree/`, and a completion file `review.completion.json` bound to the exact candidate and evidence hash (`contracts/workflow/reviewCompletion.schema.json`). You can type into the reviewer pane; the transcript is the record and the file is the verdict. Only a passing review permits fast-forwarding the newly created `feature/…` branch. **No merge to main and no push.**
...
Do not repeat `launch` for an existing run. Uncertain launches still require explicit reconciliation. New files include `ui.completion.json`, `adapter.completion.json`, `automatic-review.json`, `review.interactive.json`, `review.launch.log`, `review.completion.json`, `review.stop.json`, `review.json`, `review.diff` and the isolated `review-worktree/`. The reviewer has Read/Glob/Grep, plus Write allow-listed to its single completion file; no shell tools. `--reviewer-transport print` restores the headless `claude --print` reviewer (adds `review.stdout.json`, `review.stderr.log`). Resume the reviewer transcript afterwards with `cd "$RUN/review-worktree" && claude --resume <uuid from review.interactive.json>`. Synthetic tests of this path are not live Claude validation.
