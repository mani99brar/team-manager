...
`feature.json` (version 2.0.0, `contracts/workflow/feature.schema.json`) declares the lanes and their task files: `ui` (`ui-task.md`) and `adapter` (`adapter-task.md`). `policy.json` (version 1.2.0) gives each lane its role label, owned paths, checks and `required_check_kinds` (`ui`: `build` and `browser`; `adapter`: `unit`). `launch project-workflows --workers ui` or `--workers adapter` runs one lane alone; the other lane's owned paths stay off-limits and the candidate is built from the selected lane only. The `failure_drill` names `adapter`, so a run that leaves `adapter` out records the drill as skipped.
