...
**Export version 1.2.0** (`workflow/export_state.py`, `EXPORT_VERSION`) keeps everything above unchanged and adds two keys; the adapter accepts `1.0.0` (neither key), `1.1.0` (`review` only) and `1.2.0` (both). Each is an object or `null`, derived read-only from the run's own files; a missing file is `null` and a malformed optional receipt is `null`, never a guess:
