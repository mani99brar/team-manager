...
- **Design challenge.** A design challenge runs after preparation and before any worker launch, inside `start` and `resume` (not a LangGraph node; the exported graph shows it as the first node, `challenge`; see 4.5). One print session reads the PRD named by the feature, the tasks and `decisions.md`, and writes `challenge.json`: fragile assumptions, the strongest simpler alternative, likely failure modes, one cheap experiment that could change the choice, each concern tied to a concrete consequence with a severity. No P0 or P1: the run continues. A P0 or P1: the run pauses before any launch, and the viewer shows the concerns on the `challenge` node. The operator edits the feature files and runs `workflow resume <run>`, which commits them on the run's branch, re-pins them and reruns the challenge; or `workflow resume <run> --accept-challenge "<reason>"` records the override and continues. A feature can turn the challenge off with `"challenge": false` in `feature.json`.
...
`challenge` is the first node of a 2.2.0 run's exported graph (`run-state.json` and the registry entry): kind `review`, label "Design challenge", `depends_on: []`. Every launch node depends on it. A run whose feature sets `challenge: false`, and every 2.1.0 run, has no challenge node, so its graph is unchanged. `export_state.graph_nodes` takes the flag from the plan.

It is not a LangGraph node. The challenge runs inside `start` and `resume` (`guardrails.challenge_gate` and `guardrails.resume_challenge`), before the graph is first invoked, because it must be decided before any worker session exists and it has its own pause and resume (`challenge.json`, `resume`, `--accept-challenge`). The graph LangGraph runs starts with the `launch_<lane>` nodes, and a run whose challenge paused has no checkpoint yet. The challenge is one `claude --print` job with Read, Glob and Grep only, run in a read-only worktree at the base commit, using the existing print-job runner, with its output validated against `contracts/workflow/challenge.schema.json`. The controller writes `challenge.json` in the run directory:
...
No P0 or P1 concern: `status` is `passed`, and `start` launches the workers. A P0 or P1: `status` is `paused`, no worker is launched, the timeline records the pause, and the launch exits 0 with the concerns and the resume commands printed. `python -m workflow resume <run>` re-reads the task files, `decisions.md` and the PRD from the paths pinned at prepare, commits the changed ones on the run's branch, moves the run to that commit, updates the plan's pinned copies, and reruns the challenge as the next attempt; the source checkout stays clean for integration. Only those paths may be changed in the source checkout; anything else is refused and listed, and so is a run worktree that is not a clean checkout of the base, before anything is committed. The commit (`Workflow <run>: feature files revised after design challenge attempt <n>`, the pipeline's own identity, hooks off) sits on the base the plan pinned. Every worktree of the run (the lanes' and the challenge's) checks out the commit, and the plan's `base_commit` and each lane's `observed_start_commit` follow, as a fresh prepare would pin them, in the same `plan.json` write as the re-pinned copies. No worker exists yet, so no receipt binds the old plan. `challenge-revision.json` marks the move from before the commit until that write; while it exists `start` and `--accept-challenge` refuse, so no worker launches on a half-moved run, and a rerun of `resume` continues from the commit already made. `resume <run> --accept-challenge "<reason>"` records `accepted` with the reason and continues without rerunning; it refuses while a pinned feature file no longer holds the plan's copy or an interrupted `resume` has not finished, and when the paused attempt's `pinned` digests are not those of the plan's copies (a rerun failed after its re-pin), so the workers never run on files no challenge read. The policy is never re-pinned; changing it needs a new run. `disabled` is written when the feature turns the challenge off. Earlier attempts are kept as `challenge-<attempt>.json`.
