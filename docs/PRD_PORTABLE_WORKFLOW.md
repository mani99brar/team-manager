...
No P0 or P1 concern: `status` is `passed`, and `start` launches the workers. A P0 or P1: `status` is `paused`, no worker is launched, the timeline records the pause, and the launch exits 0 with the concerns and the resume commands printed. `python -m workflow resume <run>` re-reads the task files, `decisions.md` and the PRD from the paths pinned at prepare, commits the changed ones on the run's branch, moves the run to that commit, updates the plan's pinned copies, and reruns the challenge as the next attempt; the source checkout stays clean for integration. Only those paths may be changed in the source checkout; anything else is refused and listed. The commit (`Workflow <run>: feature files revised after design challenge attempt <n>`, the pipeline's own identity, hooks off) sits on the base the plan pinned. Every worktree of the run (the lanes' and the challenge's) must be a clean checkout of that base; each checks out the commit, and the plan's `base_commit` and each lane's `observed_start_commit` follow, as a fresh prepare would pin them. No worker exists yet, so no receipt binds the old plan. A `resume` interrupted after its commit continues from it when rerun; the challenge stays paused meanwhile, so no worker launches on a half-moved run. `resume <run> --accept-challenge "<reason>"` records `accepted` with the reason and continues without rerunning; it refuses while a pinned feature file is edited or an interrupted `resume` left a revision the run does not use yet. The policy is never re-pinned; changing it needs a new run. `disabled` is written when the feature turns the challenge off. Earlier attempts are kept as `challenge-<attempt>.json`.
