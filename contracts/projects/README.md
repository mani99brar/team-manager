# Project workflow viewer contract v1.3.0
...
| `GET .../runs/{run_id}/reviews/{attempt}` | `reviewResult.schema.json`: the persisted review verdict, reviewer identity, bundle hash, findings and the diff artifact (1.1.0; finding links 1.2.0; lane attributions 1.3.0) |
| `GET .../runs/{run_id}/inputs` | `runInputs.schema.json`: what the run was asked to do, pinned from its own files (1.2.0; selected and excluded lanes 1.3.0) |
