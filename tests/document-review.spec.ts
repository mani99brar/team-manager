...
import { CLAUDE, PI, browseUrl, expandButton, collapseButton, expandLocation, fileUrl, fit, node, nodeBody, openGraph, visibleNodeIds, viewTransform, dragNode, waitForApi } from './helpers.ts'
...
for (const entry of ['direct', 'reload', 'retry'] as const) {
  test(`${entry} announces loading in the live region`, async ({ page }) => {
    const url = fileUrl('Pi', PI, 'workflow.md')
    if (entry === 'retry') {
...
test('browser Back restores focus without reopening a collapsed selected folder or moving the viewport', async ({ page }) => {
  ...
  await nodeBody(page, `Claude/${CLAUDE}/workflow.md`).focus()
...
test('repeated document URLs keep independent origin selection, mode, expansion, positions, pins and viewport', async ({ page }) => {
  ...
  await fit(page)
  await dragNode(page, `Pi/${PI}/workflow.md`, 45, 30)
  await page.getByRole('button', { name: 'Zoom out' }).click()
  const ids = await visibleNodeIds(page)
  const view = await viewTransform(page)
  ...
  await outline.getByRole('button', { name: /^Claude\s*, source$/ }).click()
  await outline.locator(`[data-node-id="Pi/${PI}/workflow.md"] .outline-name`).click()
  await expect(page.getByRole('tabpanel')).toContainText('Sample Pi workflow')
  await page.goBack() // /browse/Claude origin of the second opening
  await expect(outline).toBeVisible()
  await expect(outline.locator(`[data-node-id="Pi/${PI}/workflow.md"] .outline-name`)).toBeFocused()
